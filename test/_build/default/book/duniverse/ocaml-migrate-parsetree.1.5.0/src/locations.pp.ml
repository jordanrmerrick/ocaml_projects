# 1 "book/duniverse/ocaml-migrate-parsetree.1.5.0/src/locations.ml"
...
type location_report                   = Location.report    = {
  kind : location_report_kind;
  main : location_msg;
  sub : location_msg list;
}

type location_error                   = Location.error    (*IF_NOT_AT_LEAST 408 = old_location_error *)
...
let location_error_of_exn : exn -> location_error = fun exn ->
                    match Location.error_of_exn exn with None | Some `Already_displayed -> raise exn | Some (`Ok e) -> e   
...
let get_error_message location_error =
  (*IF_NOT_AT_LEAST 408 _get_error_message_old location_error*)
                    _get_error_message_new location_error  
...
let set_error_message location_error msg =
  (*IF_NOT_AT_LEAST 408 _set_error_message_old location_error msg*)
                    _set_error_message_new location_error msg  
